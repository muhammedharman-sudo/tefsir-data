...
async function _notlarGithubSenkronla(sureNo) {
  ...
  try {
    ...
    const tamMetinSure = parcalar.join('\n\n');

    // ÖNEMLİ: Bu sure hangi KAYITLI kaynağa (GitHub'daki hangi gerçek dosyaya) ait, onu bul.
    // "Tüm Kur'an tek dosya" (çoklu) modundaysanız, ayrı bir "4.txt" YAZMAK YERİNE
    // o TEK dosyanın sadece bu sureye ait bölümünü güncelliyoruz — yoksa yazdığınız yer,
    // okumada kullanılan dosyayla hiç ilgisi olmayan başka bir dosya olur ve değişiklik
    // "kayboluyormuş" gibi görünür.
    const kaynaklar = _tefsirKaynaklariGetir();
    const tekKaynak = kaynaklar.find(k => k.url && String(k.sureNo) === String(sureNo));
    const cokluKaynak = kaynaklar.find(k => k.url && !k.sureNo);
    const kaynak = tekKaynak || cokluKaynak;

    let dosyaYolu = kaynak ? _githubPathFromUrl(kaynak.url) : null;
    if (!dosyaYolu) dosyaYolu = sureNo + '.txt'; // kayıtlı kaynak yok/URL çözülemedi — son çare

    let icerikYazilacak;
    if (kaynak && !tekKaynak) {
      // ÇOKLU (tüm Kur'an tek dosya) modu: dosyanın TAMAMINI oku, sadece bu surenin
      // bölümünü değiştirip geri kalanı olduğu gibi bırak.
      const mevcut = await _githubDosyaOku(dosyaYolu);
      if (mevcut.icerik) {
        const parcalarSure = _alimTefsirCoklaAyir(mevcut.icerik);
        let bulunduMu = false;
        const yeniParcalar = parcalarSure.map(p => {
          if (p.sureNo !== parseInt(sureNo)) return p;
          bulunduMu = true;
          const basliksatiri = p.metin.split('\n')[0]; // "N- ... Sûresi" başlığını koru
          return { sureNo: p.sureNo, metin: basliksatiri + '\n\n' + tamMetinSure };
        });
        if (!bulunduMu) yeniParcalar.push({ sureNo: parseInt(sureNo), metin: sureNo + '-) Sure ' + sureNo + '\n\n' + tamMetinSure });
        icerikYazilacak = yeniParcalar.map(p => p.metin).join('\n');
      } else {
        // Dosya hiç yoksa (beklenmedik durum) en azından bu sureyi tek başına yaz
        icerikYazilacak = tamMetinSure;
      }
    } else {
      // Bu sureye özel ayrı dosya (veya hiç kaynak kaydı yok, eski/varsayılan davranış)
      icerikYazilacak = tamMetinSure;
    }

    await _githubDosyaGuncelle(dosyaYolu, icerikYazilacak, 'Notlar güncellendi: Sure ' + sureNo);
