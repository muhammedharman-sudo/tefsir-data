...
function _notlardanAlimIsimleriGetir() {
  ...
  // "Tefsir Kaynakları" ile toplu yüklenmiş (RAM'deki) veri varsa, hiç manuel not girilmemiş olsa bile
  // "Tefsir" ismini listeye ekle — böylece toplu yüklenen tüm sureler de bu sekmede görünür.
  if (window._bellekTefsirVerisi && Object.keys(window._bellekTefsirVerisi).length > 0) {
    isimler.add('Tefsir');
  }
  return [...isimler];
}
...
function _notlardanSureVerisiGetir(alim, sureNo) {
  ...
  // "Tefsir" için önce bellekteki (GitHub'dan toplu yüklenmiş) veriyi taban olarak al.
  if (alim === 'Tefsir' && window._bellekTefsirVerisi && window._bellekTefsirVerisi[sureNo]) {
    Object.assign(sonuc, window._bellekTefsirVerisi[sureNo]);
  }
  ...
  for (let a = 1; a <= ayetSayisi; a++) {
    ...
    try {
      ...
      // Yerel not, bellekteki (toplu) veriden daha güncel kabul edilir — üzerine yazar.
...
function _notlardanAlimSureleriGetir(alim) {
  ...
  // "Tefsir" için: bellekte (GitHub'dan toplu yüklenmiş) sureleri de ekle —
  // kullanıcı hiç tek tek düzenlemese bile, toplu yüklediği TÜM sureler burada görünsün.
  if (alim === 'Tefsir' && window._bellekTefsirVerisi) {
    Object.keys(window._bellekTefsirVerisi).forEach(sNoStr => {
      const sNo = parseInt(sNoStr);
      if (!isNaN(sNo) && window._bellekTefsirVerisi[sNoStr] && Object.keys(window._bellekTefsirVerisi[sNoStr]).length > 0) {
        sureler.add(sNo);
      }
    });
  }
...
function geminiAyarlariAc() {
  document.getElementById('gemini-key-modal').style.display = 'flex';
  document.getElementById('gemini-key-overlay').style.display = 'block';
  document.getElementById('gemini-key-input').value = localStorage.getItem('gemini_api_key') || '';
}

function geminiAyarlariKapat() {
  document.getElementById('gemini-key-modal').style.display = 'none';
  document.getElementById('gemini-key-overlay').style.display = 'none';
}

function geminiAnahtariKaydet() {
  const key = document.getElementById('gemini-key-input').value.trim();
  if (!key) { alert('API anahtarı gerekli.'); return; }
  localStorage.setItem('gemini_api_key', key);
  geminiAyarlariKapat();
  alert('✅ Gemini API anahtarı kaydedildi.');
}

function _geminiHazirMi() {
  return !!localStorage.getItem('gemini_api_key');
}

// 114 sureyi (1.txt ... 114.txt) GitHub'daki repo'nuzdan tek tek çekip hepsini
// IndexedDB'ye kalıcı olarak kaydeder. Bundan sonra internet olmasa bile tefsirler
// anında (cihazdan) görüntülenir. Sadece düzenleme yapıp GitHub'a göndermek için internet gerekir.
async function _tefsirTumSureleriCihazaIndir() {
  if (!_githubHazirMi()) { alert('Önce token/repo girip Kaydet\'e basın.'); return; }
  const onay = confirm('114 sure GitHub\'dan çekilip cihazınıza (IndexedDB) kalıcı olarak kaydedilecek. Biraz sürebilir (yaklaşık 1-2 dakika). Devam edilsin mi?');
  if (!onay) return;

  const tabanUrl = _notlarGithubTabanUrlGetir();
  let basarili = 0, bos = 0, hatali = 0;

  for (let sureNo = 1; sureNo <= 114; sureNo++) {
    try {
      const r = await fetch(_tazeUrl(tabanUrl + sureNo + '.txt'));
      if (!r.ok) { bos++; continue; }
      const metin = await r.text();
      const sayi = _githubTefsirYukle(sureNo, metin); // bu zaten IndexedDB'ye de yazıyor
      if (sayi > 0) basarili++; else bos++;
    } catch(e) {
      hatali++;
    }
  }

  alert('✅ İndirme tamamlandı!\n\nBaşarıyla indirilen: ' + basarili + ' sure\nBoş/bulunamadı: ' + bos + ' sure\nHata: ' + hatali + ' sure\n\nArtık bu sureler internetsiz de anında açılacak.');
}

// Kullanıcının token/repo bilgilerinin GERÇEKTEN çalışıp çalışmadığını adım adım test eder
// ve GitHub'ın döndürdüğü TAM hata mesajını gösterir (genel "ağ hatası" değil).
async function _githubBaglantiyiTestEt() {
  const token = document.getElementById('github-token-input').value.trim();
  const repo = document.getElementById('github-repo-input').value.trim();
  if (!token || !repo) { alert('Önce token ve repo adını girin.'); return; }

  const sonuclar = [];

  // 1) Token geçerli mi? (kullanıcı bilgisi)
  try {
    ...
    if (r.ok) {
      ...
      sonuclar.push('✅ Token geçerli. GitHub kullanıcı adı: ' + d.login);
    } else {
      const d = await r.json().catch(()=>({}));
      sonuclar.push('❌ Token hatası (HTTP ' + r.status + '): ' + (d.message || 'bilinmeyen'));
    }
  } catch(e) {
    sonuclar.push('❌ Token kontrolü sırasında AĞ HATASI (fetch tamamlanamadı): ' + e.message);
  }

  // 2) Repo'ya erişim var mı?
  try {
    ...
    if (r2.ok) {
      ...
      sonuclar.push('✅ Repo bulundu: ' + d2.full_name + ' (' + (d2.private ? 'private' : 'public') + ')');
    } else {
      const d2 = await r2.json().catch(()=>({}));
      sonuclar.push('❌ Repo hatası (HTTP ' + r2.status + '): ' + (d2.message || 'bilinmeyen') + ' — repo adını "' + repo + '" kontrol edin.');
    }
  } catch(e) {
    sonuclar.push('❌ Repo kontrolü sırasında AĞ HATASI (fetch tamamlanamadı): ' + e.message);
  }

  // 3) Yazma yetkisi var mı? (deneme dosyası oluşturup hemen sil)
  try {
    const testYolu = '_baglanti_testi_' + Date.now() + '.txt';
    const apiUrl = 'https://api.github.com/repos/' + repo + '/contents/' + testYolu;
    const putR = await fetch(apiUrl, {
      method: 'PUT',
      headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: 'Bağlantı testi', content: btoa('test') })
    });
    if (putR.ok) {
      sonuclar.push('✅ Yazma yetkisi çalışıyor (test dosyası oluşturuldu ve silinecek).');
      const putD = await putR.json();
      // Test dosyasını hemen sil
      await fetch(apiUrl, {
        method: 'DELETE',
        headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github+json', 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: 'Test dosyası temizlendi', sha: putD.content.sha })
      }).catch(()=>{});
    } else {
      const putD = await putR.json().catch(()=>({}));
      sonuclar.push('❌ Yazma hatası (HTTP ' + putR.status + '): ' + (putD.message || 'bilinmeyen'));
    }
  } catch(e) {
    sonuclar.push('❌ Yazma testi sırasında AĞ HATASI (fetch tamamlanamadı): ' + e.message);
  }

  alert(sonuclar.join('\n\n'));
}

// Kayıtlı kaynak URL'sinden (raw.githubusercontent.com veya github.com/blob) repo içindeki
// gerçek dosya yolunu çıkarır. Böylece yazma işlemi, OKUMADA kullanılan dosyayla AYNI dosyaya gider.
function _githubPathFromUrl(url) {
  try {
    const u = new URL(url);
    const parcalar = u.pathname.split('/').filter(Boolean).map(p => {
      try { return decodeURIComponent(p); } catch(e) { return p; }
    });
    if (u.hostname === 'raw.githubusercontent.com') {
      // /{owner}/{repo}/{branch}/{...path}
      if (parcalar.length >= 4) return parcalar.slice(3).join('/');
    }
    if (u.hostname === 'github.com') {
      // /{owner}/{repo}/blob/{branch}/{...path}
      const blobIdx = parcalar.indexOf('blob');
      if (blobIdx >= 0 && parcalar.length > blobIdx + 2) return parcalar.slice(blobIdx + 2).join('/');
    }
  } catch(e) {}
  return null;
}

// dosyaYolu'ndaki her klasör/dosya parçasını API isteğine güvenli şekilde eklemek için kodlar
// (boşluk, Türkçe karakter vb. içeren dosya adları için gereklidir).
function _githubApiYolKodla(dosyaYolu) {
  return dosyaYolu.split('/').map(encodeURIComponent).join('/');
}

// Bir dosyayı GitHub'dan (API üzerinden, CDN önbelleği OLMADAN — her zaman en güncel) okur.
// Dönen: {icerik, sha} — dosya yoksa {icerik: null, sha: null}
async function _githubDosyaOku(dosyaYolu) {
  const token = localStorage.getItem('github_pat');
  const repo = localStorage.getItem('github_repo');
  if (!token || !repo) throw new Error('GitHub token/repo ayarlanmamış.');
  const apiUrl = `https://api.github.com/repos/${repo}/contents/${_githubApiYolKodla(dosyaYolu)}`;
  const r = await fetch(apiUrl, {
    headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github+json' }
  });
  if (r.status === 404) return { icerik: null, sha: null };
  if (!r.ok) throw new Error('Dosya okunamadı (' + r.status + ')');
  const data = await r.json();
  const base64 = (data.content || '').replace(/\n/g, '');
  const binStr = atob(base64);
  const bytes = new Uint8Array(binStr.length);
  for (let i = 0; i < binStr.length; i++) bytes[i] = binStr.charCodeAt(i);
  const icerik = new TextDecoder('utf-8').decode(bytes);
...
async function _githubDosyaGuncelle(dosyaYolu, yeniIcerik, commitMesaji, _tekrarMi) {
  ...
  let sha; // undefined kalırsa GitHub bunu "yeni dosya oluştur" olarak yorumlar
  if (getResp.status === 404) {
    // Dosya henüz repoda yok — ilk kez bu sure için oluşturuluyor, sha GÖNDERİLMEZ.
    sha = undefined;
  } else if (!getResp.ok) {
    throw new Error('Dosya bilgisi alınamadı (' + getResp.status + '). Dosya yolu doğru mu?');
  } else {
  ...
  try {
    ...
    if (sha) govde.sha = sha; // yeni dosyada sha gönderilmez
...
async function _bekleyenGithubYazmalariniDene() {
  ...
  // ve biraz sonra GitHub'a tekrar göndermeyi denediğimizde doğru (güncel) metni göndeririz.
  ...
  for (const sureNo of sureNolar) {
    try {
      // _notlarGithubSenkronla, kayıtlı kaynağa göre doğru dosyayı kendisi bulur
      // (tekil dosya mı, çoklu/bulk dosyanın bir bölümü mü) ve içeriği (biraz önce belleğe
      // uyguladığımız güncel metinden) yeniden üretip gönderir.
      await _notlarGithubSenkronla(sureNo);
...
// Sizin ayarladığınız repo'ya göre taban URL'yi otomatik oluşturur (sabit/hardcoded değil artık).
// Böylece hangi repo'yu "GitHub Token Ayarla" ekranında girdiyseniz, okuma da HEP o repo'dan olur.
function _notlarGithubTabanUrlGetir() {
  const repo = localStorage.getItem('github_repo');
  if (repo) return 'https://raw.githubusercontent.com/' + repo + '/main/';
  return 'https://raw.githubusercontent.com/muhammedharman-sudo/tefsir-data/main/'; // son çare / varsayılan
}
...
async function _notlarGithubSenkronla(sureNo) {
  ...
  try {
    ...
    tamMetinSure = parcalar.join('\n\n');

    // BASİT VE SAĞLAM KURAL: her sure kendi küçük dosyasında tutulur, dosya adı = "sureNo.txt".
    // Koca bir dosyanın içini bulup değiştirmeye ÇALIŞMIYORUZ artık — bu yaklaşım kırılgandı
    // (dosya biraz bozulsa/formatı sapsa her şey kilitleniyordu). Artık her sure tamamen
    // bağımsız, küçük ve basit bir dosya; bozulma riski neredeyse yok.
    const dosyaYolu = sureNo + '.txt';

    await _githubDosyaGuncelle(dosyaYolu, tamMetinSure, 'Notlar güncellendi: Sure ' + sureNo);
    // Başarılı — eğer bu sure daha önce bekleyen kuyrukta ise temizle
    const bekleyenler = _bekleyenGithubYazmalariGetir();
    ...
    // Yazılamadı — değişikliğiniz KAYBOLMASIN diye cihazda "yeniden dene" kuyruğuna alınıyor.
    // ÖNEMLİ: Bazı düzenlemeler (GitHub kaynaklı sureler) sadece BELLEKTE tutulur, localStorage'a
    // hiç yazılmaz — bu yüzden sayfa yenilenirse bu düzenleme tamamen kaybolur. Bunu önlemek için
    // metnin kendisini de (sadece sure numarasını değil) kuyruğa kaydediyoruz.
    try {
      if (tamMetinSure) {
        const bekleyenler = _bekleyenGithubYazmalariGetir();
        bekleyenler[sureNo] = tamMetinSure;
        _bekleyenGithubYazmalariKaydet(bekleyenler);
      }
    } catch(e2) { /* kuyruğa alma da başarısız olursa sessiz geç */ }
...
function _bellekTefsirAyetKaydet(sureNo, ayetNo, icerik) {
  ...
  // KALICI kayıt: RAM'deki bu değişikliği IndexedDB'ye de yaz — sayfa yenilense/telefon
  // kapansa bile bu düzenleme ARTIK KAYBOLMAZ (GitHub'a yazma başarısız olsa dahi).
  if (typeof _tefsirDBYaz === 'function') {
    _tefsirDBYaz(sureNo, window._bellekTefsirVerisi[sureNo]).catch(()=>{});
  }
}
...
async function _bellekTefsirSureCek(sureNo) {
  const NOTLAR_GITHUB_TABAN_URL_YEREL = _notlarGithubTabanUrlGetir();
...
async function _bellekTefsirSureHazirla(sureNo, tazelemeCB) {
  ...
  // ÖNCE IndexedDB'ye bak (kalıcı, cihazda, internet gerekmez, anında gelir).
  // Bu bulunursa GitHub'a HİÇ gitmeye gerek yok — hem hızlı hem çevrimdışı çalışır.
  let basarili = false;
  if (typeof _tefsirDBOku === 'function') {
    try {
      const kayitli = await _tefsirDBOku(sureNo);
      if (kayitli && Object.keys(kayitli).length > 0) {
        if (!window._bellekTefsirVerisi) window._bellekTefsirVerisi = {};
        window._bellekTefsirVerisi[sureNo] = kayitli;
        if (!window._bellekTefsirKaynaklar) window._bellekTefsirKaynaklar = {};
        window._bellekTefsirKaynaklar[sureNo] = true;
        basarili = true;
      }
    } catch(e) { /* IndexedDB yoksa/hata verirse sessizce GitHub'a düş */ }
  }

  // IndexedDB'de yoksa, GitHub'dan çek (ve az sonra IndexedDB'ye de yazılacak — bkz. _githubTefsirYukle)
  if (!basarili) basarili = await _bellekTefsirSureCek(sureNo);
...
function _ceviriKaynakListeRender() {
  ...
  const ipucu = document.createElement('div');
  ipucu.style.cssText = 'font-size:11.5px;color:var(--muted);padding:0 2px 10px;line-height:1.5;';
  ipucu.innerHTML = '💡 <b>Sure No</b> alanını boş bırakırsanız, dosyanın <b>tüm Kur\'an tek dosya</b> olduğu kabul edilir — her sure "<b>1- Fâtiha Sûresi</b>" gibi bir başlıkla ayrılır. Dipnotlar köşeli <code>[1]</code>, köşesiz <code>1</code> veya yıldız <code>(*)</code> — hepsi otomatik tanınır.';
  liste.appendChild(ipucu);

  kaynaklar.forEach((k, i) => {
    const satir = document.createElement('div');
    satir.style.cssText = 'border:1px solid var(--border);border-radius:10px;padding:10px;margin-bottom:10px;';
    const cokluMu = !k.sureNo;
    satir.innerHTML = `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
      ...
      <input data-idx="${i}" data-field="sureNo" type="number" min="1" max="114" value="${k.sureNo||''}"
        placeholder="Sure No (boş=tüm Kur'an)"
        style="width:100%;padding:7px 10px;border:1px solid var(--border);border-radius:8px;background:var(--paper2);font-family:'Source Serif 4',serif;font-size:13px;color:var(--ink);box-sizing:border-box;outline:none;margin-bottom:8px;">
      <input data-idx="${i}" data-field="url" value="${k.url||''}"
        placeholder="https://raw.githubusercontent.com/..."
        style="width:100%;padding:7px 10px;border:1px solid var(--border);border-radius:8px;background:var(--paper2);font-family:'Source Serif 4',serif;font-size:12px;color:var(--ink);box-sizing:border-box;outline:none;margin-bottom:8px;">
      <button onclick="_tekCeviriYukle(${i})" style="width:100%;padding:8px;border:none;border-radius:8px;background:var(--ink);color:var(--gold2);font-family:'Source Serif 4',serif;font-size:13px;font-weight:600;cursor:pointer;">⬇ ${cokluMu ? 'Tüm Kur\'an\'ı Yükle' : 'Bu Meali Yükle'}</button>`;
    liste.appendChild(satir);
  });
}
...
async function _tekCeviriYukle(idx) {
  ...
  if (!isim || !url) { alert('Mütercim adı ve URL giriniz.'); return; }

  const kaynaklar = _ozelCeviriKaynaklariGetir();
  if (kaynaklar[idx]) { kaynaklar[idx].isim = isim; kaynaklar[idx].sureNo = sureNoStr; kaynaklar[idx].url = url; }
  _ozelCeviriKaynaklariKaydet(kaynaklar);

  const coklu = !sureNoStr;
  const onayMesaji = coklu
    ? isim + ' — Tüm Kur\'an (tek dosya) GitHub\'dan çekilecek. Devam edilsin mi?'
    : isim + ' — Sure ' + sureNoStr + ' meali GitHub\'dan çekilecek ve üzerine yazılacak. Devam edilsin mi?';
  const onay = confirm(onayMesaji);
  if (!onay) return;
  ...
  try {
    ...
    if (coklu) {
      const { sureSayisi, ayetSayisi } = _ozelCeviriCokluYukle(isim, metin);
      Object.keys(cevirilerCache).forEach(k => delete cevirilerCache[k]);
      if (sureSayisi === 0) {
        alert('⚠️ Hiçbir sure ayrıştırılamadı. Dosyanın "N- Sûre Adı Sûresi" başlıklarıyla ayrıldığından emin olun.');
      } else {
        alert('✅ ' + isim + ' — ' + sureSayisi + ' sure, toplam ' + ayetSayisi + ' ayet yüklendi!');
      }
    } else {
      const sureNo = parseInt(sureNoStr);
...
function ceviriKaynaklariniKaydet() {
  ...
  sureInputs.forEach(inp => { const idx = parseInt(inp.dataset.idx); if (kaynaklar[idx]) kaynaklar[idx].sureNo = inp.value.trim(); });
  urlInputs.forEach(inp => { const idx = parseInt(inp.dataset.idx); if (kaynaklar[idx]) kaynaklar[idx].url = inp.value.trim(); });
  // NOT: sureNo kasıtlı olarak boş bırakılabilir ("tüm Kur'an tek dosya" modu) — sadece isim+url yeterli.
  const temiz = kaynaklar.filter(k => k.isim && k.url);
  ...
  kaynaklar.forEach((k, i) => {
    ...
    satir.innerHTML = `
      ...
      <input data-idx="${i}" data-field="url" value="${k.url||''}"
        placeholder="https://raw.githubusercontent.com/..."
        style="width:100%;padding:7px 10px;border:1px solid var(--border);border-radius:8px;background:var(--paper2);font-family:'Source Serif 4',serif;font-size:12px;color:var(--ink);box-sizing:border-box;outline:none;margin-bottom:8px;">
      <button onclick="_tekSureYukle(${i})" style="width:100%;padding:8px;border:none;border-radius:8px;background:var(--ink);color:var(--gold2);font-family:'Source Serif 4',serif;font-size:13px;font-weight:600;cursor:pointer;margin-bottom:${cokluMu ? '8px' : '0'};">⬇ Bu Kaynağı Yükle</button>
      ${cokluMu ? `<button onclick="_bulkDosyayi114SureyeBol(${i})" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:8px;background:none;font-family:'Source Serif 4',serif;font-size:12.5px;font-weight:600;color:var(--ink);cursor:pointer;">🔀 114 Sureye Böl ve GitHub'a Yükle (bir kerelik)</button>` : ''}`;
    liste.appendChild(satir);
  });
}

// Tek bir büyük "tüm Kur'an" dosyasını 114 sureye bölüp, her birini kendi küçük dosyası
// (1.txt, 2.txt, ... 114.txt) olarak GitHub'a yükler. BİR KERELİK bir taşıma işlemidir.
// Bundan sonra hem okuma hem yazma bu küçük dosyalar üzerinden, çok daha sağlam şekilde çalışır.
async function _bulkDosyayi114SureyeBol(idx) {
  if (!_githubHazirMi()) { alert('Önce "Daha → GitHub Token Ayarla" ekranından token/repo girin.'); return; }
  const kaynaklar = _tefsirKaynaklariGetir();
  const k = kaynaklar[idx];
  if (!k || !k.url) { alert('Önce URL girin.'); return; }

  const onay = confirm('Bu işlem, "' + k.url + '" dosyasındaki TÜM sureleri okuyup, her birini GitHub reponuzda ayrı bir dosya (1.txt, 2.txt, ... 114.txt) olarak oluşturacak/güncelleyecek. Sure sayısına göre biraz sürebilir. Devam edilsin mi?');
  if (!onay) return;

  try {
    const r = await fetch(_tazeUrl(k.url));
    if (!r.ok) { alert('Kaynak dosya okunamadı: ' + r.status); return; }
    const metin = await r.text();
    const parcalar = _alimTefsirCoklaAyir(metin);

    if (parcalar.length === 0) {
      alert('⚠️ Hiç sure başlığı bulunamadı, bölünecek bir şey yok.');
      return;
    }
    ...
    for (const p of parcalar) {
      try {
        await _githubDosyaGuncelle(p.sureNo + '.txt', p.metin, 'Sure ' + p.sureNo + ' — 114 dosyaya bölme işlemiyle oluşturuldu');
        basarili++;
      } catch(e) {
        hatali++;
        hataDetaylari.push('Sure ' + p.sureNo + ': ' + e.message);
      }
      // GitHub API hız sınırına takılmamak için aralara küçük bir bekleme koyuyoruz
      await new Promise(res => setTimeout(res, 300));
    }

    let mesaj = '✅ Bölme işlemi tamamlandı!\n\nBaşarılı: ' + basarili + ' sure\nHatalı: ' + hatali + ' sure';
    if (hataDetaylari.length > 0) mesaj += '\n\nHatalar:\n' + hataDetaylari.slice(0, 5).join('\n');
    alert(mesaj);

    if (basarili > 0) {
      alert('Artık "Tefsir Kaynakları" listesinden bu büyük dosyayı (Sure No boş olan satırı) silebilirsiniz — bundan sonra sureler otomatik olarak kendi küçük dosyalarından okunacak.');
    }
  } catch(e) {
...
function _ozelCeviriParseEt(metin) {
  // ÖNEMLİ: Sadece "satır başında SAYI." kalıbına bakmak yetmez — uzun dipnot metinleri
  // içinde satır başına denk gelen "54. âyet" gibi ifadeler YANLIŞLIKLA yeni bir ayet
  // sınırı sanılabiliyordu (bu yüzden "ayet numaraları kayıyor" sorunu oluyordu).
  // Çözüm: adayları bul, ama sadece SIRALI (1, 2, 3, ...) olanları gerçek ayet sınırı kabul et.
  const adayRegex = /(?:^|\n)\s*((?:\d+,\s*)*\d+)\.\s+/g;
  const adaylar = [];
  let am;
  while ((am = adayRegex.exec(metin)) !== null) {
    const numaralar = am[1].split(',').map(s => parseInt(s.trim()));
    adaylar.push({ index: am.index, tamEslesme: am[0], ilkNo: numaralar[0], sonNo: numaralar[numaralar.length-1], anahtar: am[1].replace(/\s+/g,'') });
  }

  // Sıralı doğrulama: ilk aday her ne olursa olsun başlangıç kabul edilir,
  // sonraki her aday tam olarak (bir önceki sonNo + 1) olmalı — yoksa reddedilir (footnote içi rastgele sayı).
  const kabulEdilenler = [];
  let beklenen = null;
  for (const a of adaylar) {
    if (beklenen === null || a.ilkNo === beklenen) {
      kabulEdilenler.push(a);
      beklenen = a.sonNo + 1;
    }
    // beklenen'e uymayan adaylar sessizce atlanır (footnote/metin içi tesadüfi sayı)
  }

  const sonuc = {};
  for (let i = 0; i < kabulEdilenler.length; i++) {
    const bas = kabulEdilenler[i];
    const bitIndex = (i + 1 < kabulEdilenler.length) ? kabulEdilenler[i+1].index : metin.length;
    const ayetKey = bas.anahtar;
    let blok = metin.slice(bas.index + bas.tamEslesme.length, bitIndex).trim();
    if (!blok) continue;
...
// ════════════════════════════════════════
//  ÇOKLU SURE (TÜM KUR'AN TEK DOSYA) — MEAL
//  Format: her sure "N- Sûre Adı Sûresi" başlığıyla başlar, altında "1. metin[1]" tarzı
//  ayetler ve dipnotlar (köşeli/köşesiz/yıldız — _ozelCeviriParseEt zaten hepsini destekliyor).
// ════════════════════════════════════════
function _ozelCeviriCokluYukle(isim, metin) {
  const parcalar = _alimTefsirCoklaAyir(metin); // sure başlıklarına göre böl (aynı ayırıcı yeniden kullanılıyor)
  let sureSayisi = 0, ayetSayisi = 0;

  parcalar.forEach(p => {
    const sayi = _ozelCeviriYukle(isim, p.sureNo, p.metin);
    if (sayi > 0) { sureSayisi++; ayetSayisi += sayi; }
  });
...
// O ayet için kayıtlı TÜM özel mütercimleri döndürür (Çeviriler/Mealler ekranına eklemek için)
function _ozelCevirilerHepsiGetir(sNo, aNo) {
  const kaynaklar = _ozelCeviriKaynaklariGetir();
  // Hem bu sureye özel kayıtlı olanlar HEM DE "tüm Kur'an tek dosya" (boş Sure No) ile
  // yüklenmiş olanlar dahil edilir — yoksa toplu yüklenen mütercimler burada hiç görünmez.
  const isimler = [...new Set(
    kaynaklar.filter(k => !k.sureNo || k.sureNo == sNo).map(k => k.isim)
  )];
  const sonuc = [];
...
// ════════════════════════════════════════
//  YAPAY ZEKA — SENTEZ + SOHBET (Google Gemini API)
// ════════════════════════════════════════

// O ayet için yüklü TÜM meal + tefsir kaynaklarını, model için okunabilir tek bir
// metin bloğuna dönüştürür. Hiç kaynak yoksa null döner.
function _aiKaynaklariTopla(sNo, aNo) {
  const sure = SURELER[sNo-1];
  const parcalar = [];

  // Meal kaynakları
  const mealler = _ozelCevirilerHepsiGetir(sNo, aNo);
  mealler.forEach(m => {
    if (m.text) parcalar.push('[MEAL — ' + m.author.name + ']\n' + m.text);
  });

  // Tefsir kaynakları (hem GitHub hem Notlar sisteminden gelenler)
  const alimler = _alimTefsirListesiGetir();
  alimler.forEach(alim => {
    const veri = _alimTefsirMetniGetir2(alim, sNo);
    const icerik = veri ? veri[aNo] : null;
    if (icerik && icerik.trim()) parcalar.push('[TEFSİR — ' + alim + ']\n' + icerik.trim());
  });

  if (parcalar.length === 0) return null;
  return {
    ayetRef: (sure ? sure.isim : sNo) + ' ' + sNo + ':' + aNo,
    kaynakMetni: parcalar.join('\n\n'),
    kaynakSayisi: parcalar.length
  };
}

// Gemini API'ye bir istek gönderir. mesajlar: [{rol:'user'|'model', metin:'...'}, ...]
// sistemTalimati: modelin genel davranışını belirleyen tek seferlik talimat.
async function _geminiApiCagir(sistemTalimati, mesajlar) {
  const apiKey = localStorage.getItem('gemini_api_key');
  if (!apiKey) throw new Error('Gemini API anahtarı ayarlanmamış. Daha → Gemini API Anahtarı.');

  const url = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=' + apiKey;

  const contents = mesajlar.map(m => ({
  ...
  try {
    ...
    throw new Error('İnternet bağlantısı kurulamadı (ağ hatası).');
  }
  ...
  if (!yanit.ok) {
    ...
    try {
      ...
      if (hataVerisi.error && hataVerisi.error.message) mesaj = hataVerisi.error.message;
    } catch(e) {}
    if (yanit.status === 400 && /API key/i.test(mesaj)) mesaj = 'API anahtarı geçersiz. Lütfen kontrol edin.';
    if (yanit.status === 429) mesaj = 'Günlük/dakikalık ücretsiz kullanım sınırına ulaşıldı. Biraz sonra tekrar deneyin.';
    throw new Error(mesaj);
  }

  const veri = await yanit.json();
  const aday = veri.candidates && veri.candidates[0];
  if (!aday || !aday.content || !aday.content.parts || !aday.content.parts[0]) {
    throw new Error('Model boş yanıt döndürdü (içerik güvenlik filtresine takılmış olabilir).');
  }
  return aday.content.parts.map(p => p.text || '').join('');
}

const AI_SISTEM_TALIMATI_SENTEZ =
  'Sen bir Kur\'an tefsiri asistanısın. Sana bir ayetin numarası ve o ayet için kullanıcının ' +
  'kendi yüklediği meal/tefsir kaynaklarının metinleri verilecek. Görevin:\n' +
  '1) Kısa bir "Ana Tema" bölümüyle başla (2-3 cümle) — ayet bağlamda ne anlatıyor, neden önemli.\n' +
  '2) Ardından, verilen HER kaynağı ayrı ayrı madde olarak özetle: "• [İsim]: [görüşü kendi cümlelerinle, 1-3 cümle]".\n' +
  '3) Son olarak kısa bir "Değerlendirme" bölümü: ortak noktalar, varsa farklılıklar, dikkat çekici ayrıntılar.\n\n' +
  'KURALLAR:\n' +
  '- SADECE verilen kaynak metinlerine dayan; onlardan UZUN ALINTI YAPMA, kendi cümlelerinle sentezle/özetle.\n' +
  '- Sabit bir kelime sayısına zorlanma; basit ayetlerde kısa, ihtilaflı/zengin ayetlerde daha dolu yaz — ama her zaman ayetin ana temasını gerçekten açıklamaya çalış.\n' +
  '- Türkçe, akıcı ve saygılı bir dille yaz. Başlıkları kalın yazma, sade metin kullan.';

const AI_SISTEM_TALIMATI_SOHBET =
  'Sen bir Kur\'an tefsiri/meal konusunda bilgili bir sohbet asistanısın. Kullanıcı belirli bir ayet ' +
  'hakkında sorular soruyor. Sana o ayet için kullanıcının kendi yüklediği meal/tefsir kaynakları da veriliyorsa, ' +
  'önce ONLARA dayanarak cevap ver ve hangi kaynaktan geldiğini belirt. Kullanıcı, kullanıcının yüklemediği bir ' +
  'müfessire (örn. Taberi, Kurtubî, Zemahşerî/Keşşâf, Râzî vb.) atıfla soru sorarsa, kendi genel bilgine dayanarak ' +
  'cevap verebilirsin AMA cevabının başında MUTLAKA şunu belirt: "⚠️ Bu bilgi sizin yüklediğiniz bir kaynaktan değil, ' +
  'genel bilgimden geliyor, teyide açık olabilir." Kısa alıntı dışında UZUN ALINTI YAPMA, kendi cümlelerinle anlat. ' +
  'Türkçe, akıcı ve saygılı bir dille yaz.';
...
function _githubTefsirYukle(sureNo, metin) {
  ...
  if (yuklenenSayisi > 0) {
    window._bellekTefsirKaynaklar[sureNo] = true;
    // KALICI kayıt: GitHub'dan gelen bu veriyi de IndexedDB'ye yaz — böylece bir sonraki
    // açılışta internet olmasa bile bu sure anında (IndexedDB'den) görüntülenebilir.
    if (typeof _tefsirDBYaz === 'function') {
      _tefsirDBYaz(sureNo, window._bellekTefsirVerisi[sureNo]).catch(()=>{});
    }
  }
...
async function _githubTefsirleriniCek() {
  ...
  for (const dosya of GITHUB_TEFSIR_DOSYALARI) {
    try {
      const r = await fetch(_tazeUrl(dosya.url));
...
async function _tumKaynaklariOnYukle() {
  ...
  try {
    ...
    for (const k of ceviriKaynaklari) {
      if (!k.isim || !k.url) continue;
      try {
        const r = await fetch(_tazeUrl(k.url));
        if (!r.ok) continue;
        const metin = await r.text();
        if (k.sureNo) {
          _ozelCeviriYukle(k.isim, k.sureNo, metin);
        } else {
          _ozelCeviriCokluYukle(k.isim, metin);
        }
    ...
    // Şu an açık olan sure ekranlarını da tazele (Arapça / Meal / Tefsir — hangisi olursa olsun
    // arka planda yüklenip bitince, ekranda boş görünen ayetler otomatik dolsun)
    const herhangiBirKaynakYuklendi =
      digerTefsirKaynaklari.length > 0 ||
      ceviriKaynaklari.length > 0 ||
      arapcaKaynaklari.length > 0 ||
      tefsirKaynaklari.length > 0;
    if (herhangiBirKaynakYuklendi && typeof _acikSureEkranlariniZorlaYenile === 'function') {
      _acikSureEkranlariniZorlaYenile();
    }
  } catch(e) {
    window._kaynaklarYukleniyor = false;
