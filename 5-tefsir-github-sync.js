...
async function _githubDosyaGuncelle(dosyaYolu, yeniIcerik, commitMesaji) {
...
async function _notlarGithubSenkronla(sureNo) {
  ...
  try {
    ...
    _notlarGithubDurumGoster('⚠️ GitHub\'a yazılamadı: ' + e.message, true);
  }
}
  ...
  kaynaklar.forEach((k, i) => {
    ...
    satir.innerHTML = `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
        <input data-idx="${i}" data-field="sureNo" type="number" min="1" max="114" value="${k.sureNo||''}"
          placeholder="Sure No"
          style="width:90px;padding:7px 10px;border:1px solid var(--border);border-radius:8px;background:var(--paper2);font-family:'Source Serif 4',serif;font-size:13px;color:var(--ink);box-sizing:border-box;outline:none;">
      ...
      <input data-idx="${i}" data-field="url" value="${k.url||''}"
        placeholder="https://raw.githubusercontent.com/..."
        style="width:100%;padding:7px 10px;border:1px solid var(--border);border-radius:8px;background:var(--paper2);font-family:'Source Serif 4',serif;font-size:12px;color:var(--ink);box-sizing:border-box;outline:none;margin-bottom:8px;">
      <button onclick="_tekArapcaYukle(${i})" style="width:100%;padding:8px;border:none;border-radius:8px;background:var(--ink);color:var(--gold2);font-family:'Source Serif 4',serif;font-size:13px;font-weight:600;cursor:pointer;">⬇ Bu Sureyi Yükle</button>`;
    liste.appendChild(satir);
  });
}
...
async function _tekArapcaYukle(idx) {
  ...
  const onay = confirm('Sure ' + sureNo + ' Arapça metni GitHub\'dan çekilecek ve mevcut Arapçanın üzerine yazılacak. Devam edilsin mi?');
  if (!onay) return;
...
function arapcaKaynaklariniKaydet() {
  ...
  sureInputs.forEach(inp => { const idx = parseInt(inp.dataset.idx); if (kaynaklar[idx]) kaynaklar[idx].sureNo = inp.value.trim(); });
  urlInputs.forEach(inp => { const idx = parseInt(inp.dataset.idx); if (kaynaklar[idx]) kaynaklar[idx].url = inp.value.trim(); });
  const temiz = kaynaklar.filter(k => k.sureNo && k.url);
...
async function _tumKaynaklariOnYukle() {
  ...
  try {
    ...
    for (const k of arapcaKaynaklari) {
      if (!k.sureNo || !k.url) continue;
      try {
        const r = await fetch(k.url);
        if (r.ok) { const metin = await r.text(); _ozelArapcaYukle(k.sureNo, metin); }
