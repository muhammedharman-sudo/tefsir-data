...
async function _jsonTefsirUrlEkle() {
  ...
  try {
    const r = await fetch(_tazeUrl(rawUrl));
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const data = await r.json();
...
// ÖNEMLİ: Service Worker (PWA çevrimdışı önbellek katmanı) TAMAMEN KALDIRILDI.
// Sebep: Eski SW, kendi güncellemesini indirmesi gereken dosyayı da eski (bozuk) önbellekten
// sunduğu için kendi kendini asla güncelleyemiyordu — bu da yapılan HER kod düzeltmesinin
// cihaza hiç ulaşmamasına sebep oluyordu. Artık her açılışta dosyalar HER ZAMAN doğrudan
// ağdan (taze) yükleniyor; hiçbir önbellek katmanı araya girmiyor.
