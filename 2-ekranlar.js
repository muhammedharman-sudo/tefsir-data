...
function ayetleriInlineYukle(sNo, bas, bit, ic){
  ...
  // Sadece sizin GitHub'dan yüklediğiniz meal + Arapça — dış API'ye ASLA gidilmez.
  Promise.all([
    _okuyanSureAl(sNo),
    _kurancilarArAl(sNo)
    ...
    if (mealData && mealData.length > 0) {
      ...
      ic.innerHTML = '<div style="text-align:center;padding:20px;color:var(--muted)">Meal/Arapça yüklenmedi.</div>';
    }
  }).catch(() => {
    ic.removeChild(yukEl);
    ic.innerHTML = '<div style="text-align:center;padding:20px;color:var(--muted)">Meal/Arapça yüklenmedi.</div>';
...
function _ayetleriInlineRender(sNo, bas, bit, ic){
  ...
  for(let a=bas; a<=bit; a++){
    ...
    if (dipnotlar.length > 0) {
      ...
      const parcalar = _dipnotParcalariniAyir(mealMetni);
      parcalar.forEach(p => {
        if (p.tip === 'dipnot') {
          const dipNo = p.no;
          ...
          mealDiv.appendChild(atifMetniParsele(p.deger));
        }
      });
    } else {
...
function ayetDetayAc(sNo, aNo){
  ...
  // Sadece sizin GitHub'dan yüklediğiniz Arapça ve seçili meal yazarı — dış API'ye gidilmez.
  ...
  const ar = ozelAyet ? ozelAyet.arabic : '';
  const meal = mealVeri ? mealVeri.text : '';
  const dipnotlar = mealVeri ? (mealVeri.footnotes || []) : [];
  onizlemeCache[cacheKey] = {ar, meal, dipnotlar};
  const onEl = document.getElementById('oniz-'+sNo+'-'+aNo);
  if(onEl) onEl.textContent = (ar ? ar.substring(0,30)+'…' : 'Arapça yüklenmedi');
  modalIcDoldur(sNo, aNo, ar || '(Arapça yüklenmedi)', meal || '(Meal yüklenmedi)', dipnotlar);
}
...
function modalIcDoldur(sNo, aNo, ar, meal, dipnotlar){
  ...
  mEtiket.textContent = (_seciliMealYazariGetir() || 'Meal') + ' Meali';
  mealKutu.appendChild(mEtiket);

  const mMetin = document.createElement('div');
  mMetin.className = 'meal-metin';

  // Dipnot numaralarını tıklanabilir yap (üstsimge rakamlar dahil)
  if (dipnotlar.length > 0) {
    const parcalar = _dipnotParcalariniAyir(meal || '');
    parcalar.forEach(p => {
      if (p.tip === 'dipnot') {
        const dipNo = p.no;
        const link = document.createElement('button');
        ...
        mMetin.appendChild(document.createTextNode(p.deger));
      }
    });
  } else {
  ...
  _okuyanSureAl(sNo).then(okuyanAyetler => {
    _kurancilarArAl(sNo).then(arData => {
      ...
      okuyanAyetler.forEach((ay, i) => {
        ...
        if (dipnotlar.length > 0) {
          _dipnotParcalariniAyir(mealMetin).forEach(p => {
            if (p.tip === 'dipnot') {
              const link = document.createElement('button');
              link.className = 'dipnot-ref-btn';
              link.textContent = '[' + p.no + ']';
              link.onclick = (e) => { e.stopPropagation(); _dipnotToggle(sNo, aNo, p.no); };
              mealDiv.appendChild(link);
            } else if (p.deger) {
              mealDiv.appendChild(document.createTextNode(p.deger));
            }
          });
        } else {
...
function kumeOkuDirekt(k, _tumAyetler) {
  ...
  if (yenidenKur) {
    ...
    // ── Meal Yazarı Seçici ──
    // GitHub'dan yüklediğiniz mütercimler arasından seçim yapın; seçiminiz hatırlanır.
    const yuklenmisMealler = _yuklenmisMealYazarlariGetir();
    if (yuklenmisMealler.length > 0) {
      const mealSeciciWrap = document.createElement('div');
      mealSeciciWrap.style.cssText = 'margin:0 12px 10px;';
      const mealSecici = document.createElement('select');
      mealSecici.style.cssText = 'display:block;width:100%;padding:9px 10px;background:var(--paper);border:1.5px solid var(--border);border-radius:9px;font-family:"Source Serif 4",serif;font-size:12.5px;font-weight:600;color:var(--ink);outline:none;';
      const seciliIsim = _seciliMealYazariGetir();

      const bosOpt = document.createElement('option');
      bosOpt.value = '';
      bosOpt.textContent = '📖 Meal seçin…';
      mealSecici.appendChild(bosOpt);

      yuklenmisMealler.forEach(isim => {
        const opt = document.createElement('option');
        opt.value = isim;
        opt.textContent = isim;
        if (isim === seciliIsim) opt.selected = true;
        mealSecici.appendChild(opt);
      });

      mealSecici.onchange = () => {
        _seciliMealYazariAyarla(mealSecici.value);
        ekran.dataset.kumeId = ''; // ekranı zorla yeniden kur
        kumeOkuDirekt(k, _tumAyetler);
      };

      mealSeciciWrap.appendChild(mealSecici);
      ekran.appendChild(mealSeciciWrap);
    }
    ...
    const modlar = [
      ...
      { id:'kelimemeali', ikon:'🔤', lbl:'Kelime Meali', aciklama:'Kelime kelime anlam' },
  ...
  _okuyanSureAl(aralik.sure).then(okuyanAyetler => {
    _kurancilarArAl(aralik.sure).then(arData => {
      ...
      if (!okuyanAyetler) {
        ...
        hata.style.cssText = 'text-align:center;padding:30px;color:var(--muted);';
        hata.textContent = 'Meal/Arapça yüklenmedi.';
      ...
      for (let x = aralik.bas - 1; x < aralik.bit && x < okuyanAyetler.length; x++) {
        ...
        // Dipnot numaralarını tıklanabilir yap (köşeli normal VEYA üstsimge rakamlı işaretler)
        if (dipnotlar.length > 0) {
          const parcalar = _dipnotParcalariniAyir(mealMetin);
          parcalar.forEach(p => {
            if (p.tip === 'dipnot') {
              const dipNo = p.no;
              const link = document.createElement('button');
              ...
              meDiv.appendChild(document.createTextNode(p.deger));
            }
          });
        } else {
        ...
        // Kelime Meali konteyneri — sadece 'kelimemeali' modunda dolar (lazy)
        const kmDiv = document.createElement('div');
        kmDiv.className = 'okuma-kelime-meali';
        kmDiv.style.display = 'none';
        blok.appendChild(kmDiv);
...
function _okumaModUygula(mod, ekran) {
  ...
  if (mod === 'mealler') {
    ...
    // Sadece GitHub'dan yüklediğiniz mütercimler — dış API'ye gidilmiyor.
    const sonuclar = ayetler.map(a => ({
      sNo: a.sNo, aNo: a.aNo,
      ceviriler: _ozelCevirilerHepsiGetir(a.sNo, a.aNo)
    }));
    {
      mDiv.innerHTML = '';
      sonuclar.forEach(({ sNo, aNo, ceviriler }) => {
        const kart = document.createElement('div');
        ...
        if (ceviriler.length === 0) {
          const bosEl = document.createElement('div');
          bosEl.style.cssText = 'padding:10px 14px;font-size:12.5px;color:var(--muted);';
          bosEl.textContent = 'Bu âyet için meal yüklenmedi.';
          kart.appendChild(bosEl);
          mDiv.appendChild(kart);
          return;
        }

        const sirali = ceviriler;
        ...
        sirali.forEach((c, i) => {
          ...
          if (dipnotlar.length > 0) {
            // [1], [2] veya üstsimge [¹⁸¹⁹] referanslarını tıklanabilir span'e çevir
            const metinHTML = (c.text || '').replace(/\[([0-9⁰¹²³⁴⁵⁶⁷⁸⁹]+)\]/g, (m, num) => {
              const gercekNo = _supRakamNormalize(num);
              return `<span class="meal-dipnot-ref" data-num="${gercekNo}" style="color:var(--gold);font-weight:700;cursor:pointer;font-size:0.85em;vertical-align:super;">[${gercekNo}]</span>`;
            });
  ...
  ekran.querySelectorAll('.okuma-ayet-blok').forEach(blok => {
    ...
    const km = blok.querySelector('.okuma-kelime-meali');
    ...
    if (mod === 'hafiza') {
      ...
      if (km) {
        km.style.display = '';
        _kelimeMealiYukleVeCiz(blok.dataset.sno, blok.dataset.ano, km);
      }
    } else {
...
// ════════════════════════════════════════
//  KELİME MEALİ — Açık Kuran API (verseparts) canlı çekim
// ════════════════════════════════════════
const _kelimeMealiCache = {}; // "sNo_aNo" -> [{arabic, translation_tr, root:{arabic,latin,id}}, ...]
const _kokDetayCache = {};    // rootId -> {mean, transcription, ...}
let _kmSeciliKelimeEl = null;

async function _kelimeMealiCek(sNo, aNo) {
  const key = sNo + '_' + aNo;
  if (_kelimeMealiCache[key]) return _kelimeMealiCache[key];
  const r = await fetch('https://api.acikkuran.com/surah/' + sNo + '/verse/' + aNo + '/verseparts');
  if (!r.ok) throw new Error('API hatası (' + r.status + ')');
  const j = await r.json();
  const parcalar = (j.data || []).slice().sort((a, b) => a.sort_number - b.sort_number);
  _kelimeMealiCache[key] = parcalar;
  return parcalar;
}

async function _kokDetayCek(rootId) {
  if (_kokDetayCache[rootId]) return _kokDetayCache[rootId];
  const r = await fetch('https://api.acikkuran.com/root/' + rootId);
  if (!r.ok) throw new Error('Kök bilgisi alınamadı');
  const j = await r.json();
  _kokDetayCache[rootId] = j.data || null;
  return _kokDetayCache[rootId];
}

async function _kelimeMealiYukleVeCiz(sNo, aNo, kmDiv) {
  if (kmDiv.dataset.yuklendi === '1') return; // zaten çizildi
  kmDiv.innerHTML = '<div class="km-yukleniyor-mini"><span class="spin" style="display:inline-block;width:14px;height:14px;vertical-align:middle;margin-right:6px;"></span>Kelimeler yükleniyor…</div>';

  try {
    const parcalar = await _kelimeMealiCek(sNo, aNo);
    if (!parcalar || parcalar.length === 0) {
      kmDiv.innerHTML = '<div class="km-yukleniyor-mini">Bu âyet için kelime meali bulunamadı.</div>';
      return;
    }
    _kelimeMealiRenderBlok(kmDiv, parcalar, sNo, aNo);
    kmDiv.dataset.yuklendi = '1';
  } catch (e) {
    kmDiv.innerHTML = '<div class="km-yukleniyor-mini">⚠️ Yüklenemedi: ' + (e && e.message ? e.message : 'bilinmeyen hata') + '<br><button class="km-tekrar-btn" style="margin-top:8px;border:1px solid var(--border);background:none;border-radius:8px;padding:4px 10px;font-size:12px;color:var(--ink);cursor:pointer;">Tekrar dene</button></div>';
    const btn = kmDiv.querySelector('.km-tekrar-btn');
    if (btn) btn.onclick = () => { kmDiv.dataset.yuklendi = ''; _kelimeMealiYukleVeCiz(sNo, aNo, kmDiv); };
  }
}

function _kelimeMealiRenderBlok(kmDiv, parcalar, sNo, aNo) {
  kmDiv.innerHTML = '';
  ...
  detay.className = 'km-detay';
  detay.style.display = 'none';

  parcalar.forEach(p => {
    const kEl = document.createElement('div');
    kEl.className = 'km-kelime' + (p.root ? ' km-kok-var' : '');

    const arEl = document.createElement('div');
    arEl.className = 'km-kelime-ar';
    arEl.textContent = p.arabic || '';

    const trEl = document.createElement('div');
    trEl.className = 'km-kelime-tr';
    trEl.textContent = p.translation_tr || '';

    kEl.appendChild(arEl);
    kEl.appendChild(trEl);

    kEl.onclick = () => _kmKelimeDetayGoster(kEl, p, detay, sNo, aNo);

    wrap.appendChild(kEl);
  });

  kmDiv.appendChild(wrap);
  kmDiv.appendChild(detay);
}

async function _kmKelimeDetayGoster(kEl, parca, detayEl, sNo, aNo) {
  // Aynı kelimeye tekrar basılırsa kapat
  if (_kmSeciliKelimeEl === kEl && detayEl.style.display !== 'none') {
    detayEl.style.display = 'none';
    kEl.classList.remove('km-secili');
    _kmSeciliKelimeEl = null;
    return;
  }
  if (_kmSeciliKelimeEl) _kmSeciliKelimeEl.classList.remove('km-secili');
  kEl.classList.add('km-secili');
  _kmSeciliKelimeEl = kEl;

  detayEl.style.display = 'block';

  if (!parca.root) {
    detayEl.innerHTML = `<div><span class="km-detay-kok-lat">${parca.arabic || ''}</span></div><div style="margin-top:4px;color:var(--muted);font-size:12.5px;">${parca.translation_tr || ''} — bu kelime için ayrı bir kök kaydı yok.</div>`;
    return;
  }

  detayEl.innerHTML = '<div class="km-yukleniyor-mini" style="padding:6px 0;">Kök bilgisi yükleniyor…</div>';

  try {
    const kok = await _kokDetayCek(parca.root.id);
    detayEl.innerHTML = `
      <div>
        <span class="km-detay-kok-ar">${parca.root.arabic || ''}</span>
        <span class="km-detay-kok-lat">${parca.root.latin || ''}</span>
      </div>
      <div style="margin-top:6px;">${parca.translation_tr ? ('Bu âyette: <b>' + parca.translation_tr + '</b>') : ''}</div>
      ${kok && kok.mean ? `<div style="margin-top:6px;color:var(--text);">${kok.mean}</div>` : ''}
    `;
  } catch (e) {
    detayEl.innerHTML = `
      <div>
      ...
      <div style="margin-top:6px;">${parca.translation_tr ? ('Bu âyette: <b>' + parca.translation_tr + '</b>') : ''}</div>
      <div style="margin-top:6px;color:var(--muted);font-size:12px;">Kökün genel anlamı yüklenemedi.</div>
    `;
  }
}

// ════════════════════════════════════════
//  YAPAY ZEKA MODALI — sentez + sohbet
// ════════════════════════════════════════
let _yzSuAn = { sNo: null, aNo: null };

function yapayZekaModalKapat(e) {
  if (e && e.target !== e.currentTarget) return;
  document.getElementById('yz-modal').classList.remove('open');
  document.body.style.overflow = '';
}

async function yapayZekaModalAc(sNo, aNo) {
  _yzSuAn = { sNo, aNo };
  const sure = SURELER[sNo-1];
  document.getElementById('yz-modal-baslik').textContent = '🤖 ' + (sure ? sure.isim : sNo) + ' ' + sNo + ':' + aNo;
  const ic = document.getElementById('yz-modal-ic');
  ic.innerHTML = '<div class="yukleniyor"><div class="spin"></div>Yükleniyor…</div>';
  document.getElementById('yz-modal').classList.add('open');
  document.body.style.overflow = 'hidden';
    ...
    ic.innerHTML = `
      <div style="text-align:center;padding:24px 16px;">
        <div style="font-size:14px;color:var(--muted);margin-bottom:14px;line-height:1.6;">
          Bu özellik için önce ücretsiz bir Gemini API anahtarı girmeniz gerekiyor.
        </div>
        <button onclick="yapayZekaModalKapat();geminiAyarlariAc();" style="padding:10px 18px;border:none;border-radius:10px;background:var(--ink);color:var(--gold2);font-family:'Source Serif 4',serif;font-size:13px;font-weight:600;cursor:pointer;">🤖 Gemini API Anahtarı Gir</button>
      </div>`;
    return;
  }

  const ayetKey = sNo + '_' + aNo;
  const kayitli = await _aiSohbetOku(ayetKey);
  _yzModalIcerigiCiz(ic, ayetKey, kayitli || { sentez: null, mesajlar: [] });
}

function _yzModalIcerigiCiz(ic, ayetKey, veri) {
  ic.innerHTML = '';

  // --- Sentez bölümü ---
  const sentezKutu = document.createElement('div');
  sentezKutu.style.cssText = 'padding:14px 16px;border-bottom:1px solid var(--border);';

  if (veri.sentez) {
    const metinEl = document.createElement('div');
    metinEl.style.cssText = 'font-family:\'Source Serif 4\',serif;font-size:14px;line-height:1.8;color:var(--ink);white-space:pre-wrap;margin-bottom:10px;';
    metinEl.textContent = veri.sentez;
    sentezKutu.appendChild(metinEl);

    const yenidenBtn = document.createElement('button');
    yenidenBtn.textContent = '🔄 Yeniden Oluştur';
    yenidenBtn.style.cssText = 'padding:7px 12px;border:1px solid var(--border);border-radius:8px;background:none;font-family:\'Source Serif 4\',serif;font-size:12px;color:var(--muted);cursor:pointer;';
    yenidenBtn.onclick = () => _yzSentezOlustur(ayetKey, true);
    sentezKutu.appendChild(yenidenBtn);
  } else {
    const aciklama = document.createElement('div');
    aciklama.style.cssText = 'font-size:13px;color:var(--muted);margin-bottom:10px;';
    aciklama.textContent = 'Bu ayet için yüklü kaynaklarınızdan bir sentez oluşturulabilir.';
    sentezKutu.appendChild(aciklama);

    const olusturBtn = document.createElement('button');
    olusturBtn.id = 'yz-sentez-olustur-btn';
    olusturBtn.textContent = '✨ Sentez Oluştur';
    olusturBtn.style.cssText = 'width:100%;padding:10px;border:none;border-radius:10px;background:var(--ink);color:var(--gold2);font-family:\'Source Serif 4\',serif;font-size:13px;font-weight:600;cursor:pointer;';
    olusturBtn.onclick = () => _yzSentezOlustur(ayetKey, false);
    sentezKutu.appendChild(olusturBtn);
  }
  ic.appendChild(sentezKutu);

  // --- Sohbet bölümü ---
  const sohbetSarmalayici = document.createElement('div');
  sohbetSarmalayici.style.cssText = 'flex:1;display:flex;flex-direction:column;min-height:0;';

  const mesajListesi = document.createElement('div');
  mesajListesi.id = 'yz-mesaj-listesi';
  mesajListesi.style.cssText = 'flex:1;overflow-y:auto;padding:12px 16px;display:flex;flex-direction:column;gap:10px;';

  (veri.mesajlar || []).forEach(m => mesajListesi.appendChild(_yzMesajBalonu(m)));
  if (!veri.mesajlar || veri.mesajlar.length === 0) {
    const bosMsj = document.createElement('div');
    bosMsj.style.cssText = 'text-align:center;color:var(--muted);font-size:12.5px;padding:10px;';
    bosMsj.textContent = 'Bu ayetle ilgili istediğinizi sorabilirsiniz (örn. "Bunu Taberi nasıl yorumluyor?").';
    mesajListesi.appendChild(bosMsj);
  }
  sohbetSarmalayici.appendChild(mesajListesi);

  const girisSatiri = document.createElement('div');
  girisSatiri.style.cssText = 'display:flex;gap:8px;padding:10px 16px;border-top:1px solid var(--border);';
  const girisKutusu = document.createElement('textarea');
  girisKutusu.id = 'yz-mesaj-girisi';
  girisKutusu.placeholder = 'Sorunuzu yazın…';
  girisKutusu.rows = 1;
  girisKutusu.style.cssText = 'flex:1;padding:9px 12px;border:1px solid var(--border);border-radius:10px;background:var(--paper2);font-family:\'Source Serif 4\',serif;font-size:13px;color:var(--ink);resize:none;outline:none;max-height:90px;';
  const gonderBtn = document.createElement('button');
  gonderBtn.id = 'yz-gonder-btn';
  gonderBtn.textContent = '➤';
  gonderBtn.style.cssText = 'padding:0 16px;border:none;border-radius:10px;background:var(--ink);color:var(--gold2);font-size:16px;cursor:pointer;flex-shrink:0;';
  gonderBtn.onclick = () => _yzMesajGonder(ayetKey);
  girisKutusu.onkeydown = (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); _yzMesajGonder(ayetKey); } };
  girisSatiri.appendChild(girisKutusu);
  girisSatiri.appendChild(gonderBtn);
  sohbetSarmalayici.appendChild(girisSatiri);

  ic.appendChild(sohbetSarmalayici);
  mesajListesi.scrollTop = mesajListesi.scrollHeight;
}

function _yzMesajBalonu(m) {
  const balon = document.createElement('div');
  const kullaniciMi = m.rol === 'user';
  balon.style.cssText = 'max-width:85%;padding:9px 12px;border-radius:12px;font-family:\'Source Serif 4\',serif;font-size:13px;line-height:1.6;white-space:pre-wrap;' +
    (kullaniciMi
      ? 'align-self:flex-end;background:var(--ink);color:var(--gold2);'
      : 'align-self:flex-start;background:var(--paper2);color:var(--ink);border:1px solid var(--border);');
  balon.textContent = m.metin;
  return balon;
}

async function _yzSentezOlustur(ayetKey, yeniden) {
  const [sNo, aNo] = ayetKey.split('_').map(Number);
  const btn = document.getElementById('yz-sentez-olustur-btn') || event?.target;
  const eskiMetin = btn ? btn.textContent : null;
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Oluşturuluyor…'; }

  try {
    const kaynaklar = _aiKaynaklariTopla(sNo, aNo);
    if (!kaynaklar) {
      alert('Bu ayet için hiç meal/tefsir kaynağı yüklenmemiş. Önce "Meal Kaynakları" veya "Tefsir Kaynakları"ndan içerik yükleyin.');
      if (btn) { btn.disabled = false; btn.textContent = eskiMetin; }
      return;
    }

    const istek = `Ayet: ${kaynaklar.ayetRef}\n\nKaynaklar:\n\n${kaynaklar.kaynakMetni}\n\nYukarıdaki kaynaklara dayanarak bu ayet için sentez oluştur.`;
    const cevap = await _geminiApiCagir(AI_SISTEM_TALIMATI_SENTEZ, [{ rol: 'user', metin: istek }]);

    const mevcut = await _aiSohbetOku(ayetKey) || { mesajlar: [] };
    mevcut.sentez = cevap;
    await _aiSohbetYaz(ayetKey, mevcut);

    const ic = document.getElementById('yz-modal-ic');
    _yzModalIcerigiCiz(ic, ayetKey, mevcut);
  } catch(e) {
    alert('⚠️ Sentez oluşturulamadı: ' + e.message);
    if (btn) { btn.disabled = false; btn.textContent = eskiMetin; }
  }
}

async function _yzMesajGonder(ayetKey) {
  const [sNo, aNo] = ayetKey.split('_').map(Number);
  const girisKutusu = document.getElementById('yz-mesaj-girisi');
  const metin = girisKutusu.value.trim();
  if (!metin) return;

  const gonderBtn = document.getElementById('yz-gonder-btn');
  girisKutusu.value = '';
  girisKutusu.disabled = true;
  if (gonderBtn) gonderBtn.disabled = true;

  const mevcut = await _aiSohbetOku(ayetKey) || { sentez: null, mesajlar: [] };
  if (!mevcut.mesajlar) mevcut.mesajlar = [];
  mevcut.mesajlar.push({ rol: 'user', metin });

  const mesajListesi = document.getElementById('yz-mesaj-listesi');
  mesajListesi.innerHTML = '';
  mevcut.mesajlar.forEach(m => mesajListesi.appendChild(_yzMesajBalonu(m)));
  const bekleBalonu = document.createElement('div');
  bekleBalonu.style.cssText = 'align-self:flex-start;color:var(--muted);font-size:12.5px;padding:6px 12px;';
  bekleBalonu.textContent = '⏳ yazıyor…';
  mesajListesi.appendChild(bekleBalonu);
  mesajListesi.scrollTop = mesajListesi.scrollHeight;

  try {
    const kaynaklar = _aiKaynaklariTopla(sNo, aNo);
    const sure = SURELER[sNo-1];
    const baglam = kaynaklar
      ? `Ayet: ${kaynaklar.ayetRef}\n\nKullanıcının yüklediği kaynaklar:\n\n${kaynaklar.kaynakMetni}\n\n---\n\n`
      : `Ayet: ${(sure ? sure.isim : sNo)} ${sNo}:${aNo}\n\n(Bu ayet için kullanıcının yüklediği bir kaynak yok.)\n\n---\n\n`;

    // Sohbet geçmişini olduğu gibi gönder, sadece EN SON (yeni) kullanıcı mesajına kaynak bağlamını ekle
    const gonderilecek = mevcut.mesajlar.map(m => ({ rol: m.rol, metin: m.metin }));
    const sonIdx = gonderilecek.length - 1;
    gonderilecek[sonIdx].metin = baglam + gonderilecek[sonIdx].metin;

    const cevap = await _geminiApiCagir(AI_SISTEM_TALIMATI_SOHBET, gonderilecek);
    mevcut.mesajlar.push({ rol: 'model', metin: cevap });
    await _aiSohbetYaz(ayetKey, mevcut);
    ...
    bekleBalonu.remove();
    const hataBalonu = document.createElement('div');
    hataBalonu.style.cssText = 'align-self:flex-start;color:var(--rust);font-size:12.5px;padding:6px 12px;';
    hataBalonu.textContent = '⚠️ ' + e.message;
    mesajListesi.appendChild(hataBalonu);
  } finally {
    girisKutusu.disabled = false;
...
function ayetButonSatiri(sNo, aNo) {
  ...
  iliskiliBtn.dataset.goster = 'derinlik'; // sadece derinlikte
  ...
  // Yapay Zeka butonu (sentez + sohbet)
  const yzBtn = document.createElement('button');
  yzBtn.className = 'ayet-ekstra-btn';
  yzBtn.id = 'yz-btn-' + sNo + '-' + aNo;
  yzBtn.dataset.goster = 'derinlik';
  yzBtn.textContent = '🤖 Yapay Zeka';
  yzBtn.onclick = () => yapayZekaModalAc(sNo, aNo);

  satir.appendChild(favBtn);
  satir.appendChild(iliskiliBtn);
  satir.appendChild(yzBtn);
  return satir;
}
