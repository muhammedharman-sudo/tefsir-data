...
// Üstsimge (superscript) Unicode rakamları normal rakama çevirir.
// Bazı meal kaynaklarında dipnot işaretleri "[¹⁸¹⁹]" gibi üstsimge rakamlarla yazılıyor,
// bazılarında ise normal "[1819]" ile — ikisini de aynı şekilde tanıyabilmek için.
const _SUPRAKAM_HARITA = { '⁰':'0','¹':'1','²':'2','³':'3','⁴':'4','⁵':'5','⁶':'6','⁷':'7','⁸':'8','⁹':'9' };
function _supRakamNormalize(s) {
  return s.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹]/g, ch => _SUPRAKAM_HARITA[ch] || ch);
}

// Meal metnini, dipnot işaretlerine (köşeli normal "[1819]" VEYA köşeli üstsimge "[¹⁸¹⁹]")
// göre parçalara ayırır. Dönen dizi: [{tip:'metin', deger}, {tip:'dipnot', no}, ...]
function _dipnotParcalariniAyir(metin) {
  if (!metin) return [];
  const regex = /\[([0-9⁰¹²³⁴⁵⁶⁷⁸⁹]+)\]/g;
  const sonuc = [];
  let sonIdx = 0, m;
  while ((m = regex.exec(metin)) !== null) {
    if (m.index > sonIdx) sonuc.push({ tip:'metin', deger: metin.slice(sonIdx, m.index) });
    sonuc.push({ tip:'dipnot', no: parseInt(_supRakamNormalize(m[1])) });
    sonIdx = m.index + m[0].length;
  }
  if (sonIdx < metin.length) sonuc.push({ tip:'metin', deger: metin.slice(sonIdx) });
  return sonuc;
}

// Bir URL'nin yol (path) kısmındaki her parçasını güvenli şekilde yeniden kodlar.
// Kullanıcı "islamoglu tüm sure.txt" gibi boşluk/Türkçe karakter içeren bir dosya adını
// URL'ye OLDUĞU GİBİ yapıştırabilir — tarayıcının fetch() fonksiyonu bunu bazen reddeder
// ("Failed to fetch"). Bu fonksiyon, zaten kodlanmış olsa da olmasa da, path'i güvenli
// (ve ÇİFT kodlanmamış) hale getirir.
function _urlGuvenliHaleGetir(url) {
  try {
    const u = new URL(url);
    u.pathname = u.pathname.split('/').map(parca => {
      if (!parca) return parca;
      let cozulmus = parca;
      try { cozulmus = decodeURIComponent(parca); } catch(e) { /* zaten çözülemeyen ise olduğu gibi bırak */ }
      return encodeURIComponent(cozulmus);
    }).join('/');
    return u.toString();
  } catch(e) {
    return url; // URL parse edilemediyse dokunma
  }
}

// ════════════════════════════════════════
//  INDEXEDDB — TEFSİR KALICI DEPOSU
//  localStorage'ın aksine (5-10MB sınır), IndexedDB çok daha büyük veri saklayabilir ve
//  RAM'in aksine sayfa yenilense/telefon kapansa bile SİLİNMEZ. Tefsir/meal/arapça verisi
//  artık burada KALICI olarak tutulur; GitHub sadece yedekleme/senkron katmanı olur.
//  ÖNEMLİ: Bu katman, mevcut not ekleme ekranlarına/butonlarına HİÇ dokunmaz — sadece
//  "veri arkada nerede duruyor" kısmını değiştirir.
// ════════════════════════════════════════
let _tefsirDB = null;
function _tefsirDBAc() {
  return new Promise((resolve, reject) => {
    if (_tefsirDB) { resolve(_tefsirDB); return; }
    if (!window.indexedDB) { reject(new Error('Bu tarayıcı IndexedDB desteklemiyor.')); return; }
    const istek = indexedDB.open('tefsirAppDB', 2);
    istek.onupgradeneeded = (e) => {
      const db = e.target.result;
      if (!db.objectStoreNames.contains('tefsirSureler')) {
        db.createObjectStore('tefsirSureler', { keyPath: 'sureNo' });
      }
      if (!db.objectStoreNames.contains('aiSohbet')) {
        db.createObjectStore('aiSohbet', { keyPath: 'ayetKey' });
      }
    };
    istek.onsuccess = (e) => { _tefsirDB = e.target.result; resolve(_tefsirDB); };
    istek.onerror = (e) => reject(e.target.error);
  });
}

// Bir surenin TÜM ayet verisini ({ayetNo: metin}) IndexedDB'ye kalıcı olarak yazar.
async function _tefsirDBYaz(sureNo, ayetlerObj) {
  try {
    const db = await _tefsirDBAc();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction('tefsirSureler', 'readwrite');
      tx.objectStore('tefsirSureler').put({ sureNo: String(sureNo), ayetler: ayetlerObj, guncellendi: Date.now() });
      tx.oncomplete = () => resolve(true);
      tx.onerror = (e) => reject(e.target.error);
    });
  } catch(e) { console.warn('IndexedDB yazma hatası:', e.message); return false; }
}

// Bir surenin verisini IndexedDB'den okur. Yoksa null döner.
async function _tefsirDBOku(sureNo) {
  try {
    const db = await _tefsirDBAc();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction('tefsirSureler', 'readonly');
      const istek = tx.objectStore('tefsirSureler').get(String(sureNo));
      istek.onsuccess = () => resolve(istek.result ? istek.result.ayetler : null);
      istek.onerror = (e) => reject(e.target.error);
    });
  } catch(e) { console.warn('IndexedDB okuma hatası:', e.message); return null; }
}

// Kayıtlı TÜM surelerin verisini tek seferde okur (uygulama açılışında hızlı yüklemek için).
async function _tefsirDBTumSureleriOku() {
  try {
    const db = await _tefsirDBAc();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction('tefsirSureler', 'readonly');
      const istek = tx.objectStore('tefsirSureler').getAll();
      istek.onsuccess = () => resolve(istek.result || []);
      istek.onerror = (e) => reject(e.target.error);
    });
  } catch(e) { console.warn('IndexedDB toplu okuma hatası:', e.message); return []; }
}

// ════════════════════════════════════════
//  AI SENTEZ + SOHBET — kalıcı depo (ayet başına: sentez metni + sohbet geçmişi)
// ════════════════════════════════════════
async function _aiSohbetOku(ayetKey) {
  try {
    const db = await _tefsirDBAc();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction('aiSohbet', 'readonly');
      const istek = tx.objectStore('aiSohbet').get(ayetKey);
      istek.onsuccess = () => resolve(istek.result || null);
      istek.onerror = (e) => reject(e.target.error);
    });
  } catch(e) { console.warn('AI sohbet okuma hatası:', e.message); return null; }
}

async function _aiSohbetYaz(ayetKey, veri) {
  try {
    const db = await _tefsirDBAc();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction('aiSohbet', 'readwrite');
      tx.objectStore('aiSohbet').put({ ayetKey, ...veri, guncellendi: Date.now() });
      tx.oncomplete = () => resolve(true);
      tx.onerror = (e) => reject(e.target.error);
    });
  } catch(e) { console.warn('AI sohbet yazma hatası:', e.message); return false; }
}
...
function _tazeUrl(url) {
  if (!url) return url;
  const guvenli = _urlGuvenliHaleGetir(url);
  const ayirici = guvenli.includes('?') ? '&' : '?';
  return guvenli + ayirici + '_cb=' + Date.now();
}
...
// "Bakara", "Enfal", "Âl-i İmrân" gibi bir sure adını normalize eder (Türkçe özel karakter/aksan farklarını tolere eder)
function _sureIsimNormalize(s) {
  return s.toLocaleLowerCase('tr')
    .replace(/[âÂ]/g, 'a').replace(/[îÎ]/g, 'i').replace(/[ûÛ]/g, 'u')
    .replace(/['’]/g, '').replace(/-/g, '').replace(/\s+/g, '')
    .trim();
}
// Sure adından (örn. "Bakara", "Enfal") sure numarasını bulur. Bulunamazsa null döner.
function _sureAdindanNoBul(ad) {
  if (!ad) return null;
  const norm = _sureIsimNormalize(ad);
  for (let i = 0; i < SURELER.length; i++) {
    if (_sureIsimNormalize(SURELER[i].isim) === norm) return i + 1;
  }
  return null;
}
...
function atifMetniParsele(metin) {
  ...
  // Grup 9-11: S:A-B salt sayı aralık
  // Grup 12-13: S:A tekli
  // Grup 14-15: (SureAdı/AyetNo) — sure adıyla, sure NUMARASI olmadan
  const regex = /\[\[(\d+):(\d+)(?:\|([^\]]+))?\]\]|\((?:bkz?\.?|krş\.?|cf\.?)\s*(\d+)[:/](\d+)\)|(?:[A-Za-zÀ-ÖØ-öø-ÿÇçĞğİıÖöŞşÜü''-]+\s+)?(\d+)[:/](\d+)[–-](\d+)|(\d+)[:/](\d+)[–-](\d+)|(\d+)[:/](\d+)(?=['a-züçşğıöü\s,;.:!?)\]]|$)|\(([A-Za-zÀ-ÖØ-öø-ÿÇçĞğİıÖöŞşÜü''’]+)\s*\/\s*(\d+)\)/gi;
  ...
  while ((eslesme = regex.exec(metin)) !== null) {
    ...
    if (eslesme[1]) {
    ...
    } else if (eslesme[14] && eslesme[15]) {
      // (SureAdı/AyetNo) — örn. (Bakara/256), (Enfal/67) — isimden sure no çözümlenir
      orijinal = eslesme[0];
      const bulunanNo = _sureAdindanNoBul(eslesme[14]);
      if (!bulunanNo) {
        parcalar.push(document.createTextNode(orijinal));
        sonIdx = eslesme.index + orijinal.length;
        continue;
      ...
      sNo = bulunanNo;
      aNo = parseInt(eslesme[15]);
      const sureBilgiAd = SURELER[sNo - 1];
      if (aNo < 1 || (sureBilgiAd && aNo > sureBilgiAd.ayet)) {
        parcalar.push(document.createTextNode(orijinal));
        sonIdx = eslesme.index + orijinal.length;
        continue;
      }
